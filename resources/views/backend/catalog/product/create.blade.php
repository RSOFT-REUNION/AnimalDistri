    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body mb-3 m-2 ml-4 mr-4">
                <form class="justify-content-center" action="{{ route('backend.catalog.products.store') }}"
                    ...
                    <div class="form-group row">
                        <div class="col">
                            ...
                            <div class="form-group">
                                <label class="form-control-label" for="name">Nom du produit <span
                                        class="small text-danger">(obligatoire)</span></label>
                            ...
                            {{-- PRIX DE BASE --}}
                            <div class="form-group">
                                <label class="form-control-label" for="price_ht">Prix de base <span
                                        class="small text-danger">(obligatoire)</span></label>
                                <div class="input-group pb-2">
                                    <button class="btn btn-primary disabled" type="button" id="button-addon1">Prix
                                        HT</button>
                                    <input type="number" id="price_ht" name="price_ht" step="0.01" min="0"
                                    ...
                                    <button class="btn btn-outline-primary disabled" type="button"
                                        id="button-addon1"><i class="fa-regular fa-euro-sign"></i></button>
                                </div>
                                {{-- TVA --}}
                                <input type="radio" class="btn-check" name="tva" id="tva_0" checked
                                    value="0">
                                ...
                                <input type="radio" class="btn-check hvr-shrink" name="tva" id="tva_850"
                                    value="850">
                        ...
                        {{-- IMAGES --}}
                      @include('backend.layouts.multiple-images-preview')
                    ...
                    <div class="form-group row">
                        <div class="col">
                            ...
                            <select
                                class="form-select tomselectmultiplecategories @error('product_categories') is-invalid @enderror"
                                multiple id="product_categories[]" name="product_categories[]">
                                    ...
                                    @if ($category_list->category_id == null)
                                        ...
                                        @foreach ($category_list->childrenCategories as $childrenCategories)
                                            <option
                                                ...
                                                value="{{ $childrenCategories->id }}">{{ $category_list->name }} ->
                                                {{ $childrenCategories->name }}</option>
                                            @foreach ($childrenCategories->childrenCategories as $childrenChildrenCategories)
                                                @if ($childrenCategories->id != $childrenChildrenCategories->id)
                                                    <option
                                                        ...
                                                        value="{{ $childrenChildrenCategories->id }}">
                                                        {{ $category_list->name }} -> {{ $childrenCategories->name }}
                                                        -> {{ $childrenChildrenCategories->name }}</option>
                                                    @foreach ($childrenChildrenCategories->childrenCategories as $fourthLevelCategory)
                                                        @if ($childrenChildrenCategories->id != $fourthLevelCategory->id)
                                                            <option
                                                                ...
                                                                value="{{ $fourthLevelCategory->id }}">
                                                                {{ $category_list->name }} ->
                                                                {{ $childrenCategories->name }} ->
                                                                {{ $childrenChildrenCategories->name }} ->
                                                                {{ $fourthLevelCategory->name }}</option>
                                                        @endif
                                                    @endforeach
                                                @endif
                                            @endforeach
